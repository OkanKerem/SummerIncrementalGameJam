...
{
    public static class O_GameSetupEditor
    {
        ...
        private const string ScenePath = "Assets/Scenes/O_Game.unity";

        [MenuItem("Universes/Old/Setup Old Game (Full)")]
        public static void SetupFullGame()
        {
            ...
            Debug.Log("Old Universes game setup complete! Open Assets/Scenes/O_Game.unity and press Play.");
        }
        ...
        private static void EnsureFolders()
        {
            ...
            {
                "Assets/Scripts", SoPath, $"{SoPath}/O_Upgrades", $"{SoPath}/O_Variants",
        ...
        private static O_GameBalance CreateOrLoadBalance()
        {
            var path = $"{SoPath}/O_GameBalance.asset";
            var balance = AssetDatabase.LoadAssetAtPath<O_GameBalance>(path);
            if (balance != null)
                return balance;

            balance = ScriptableObject.CreateInstance<O_GameBalance>();
            AssetDatabase.CreateAsset(balance, path);
            return balance;
        }

        private static O_UpgradeCatalog CreateUpgradeCatalog(O_GameBalance balance)
        {
            var path = $"{SoPath}/O_UpgradeCatalog.asset";
            var catalog = AssetDatabase.LoadAssetAtPath<O_UpgradeCatalog>(path);
            if (catalog != null && catalog.upgrades != null && catalog.upgrades.Length > 0)
                return catalog;

            var upgrades = new[]
            {
                CreateUpgrade("big_bang", "Stronger Big Bang", "+1 starting star per level", O_UpgradeEffectType.StartingStars, 1, 15),
                CreateUpgrade("cosmic_reserves", "Cosmic Reserves", "+10 starting stardust per level", O_UpgradeEffectType.StartingStardust, 10, 12),
                CreateUpgrade("entropy_dampening", "Entropy Dampening", "-10% entropy rate per level", O_UpgradeEffectType.EntropyReduction, 0.1, 20),
                CreateUpgrade("stellar_longevity", "Stellar Longevity", "+15% star lifespan per level", O_UpgradeEffectType.StarLifespan, 0.15, 18),
                CreateUpgrade("stellar_harvest", "Stellar Harvest", "+20% stardust production per level", O_UpgradeEffectType.StardustProduction, 0.2, 15),
                CreateUpgrade("click_resonance", "Click Resonance", "+25% click yield per level", O_UpgradeEffectType.ClickYield, 0.25, 12),
                CreateUpgrade("planet_forge", "Planet Forge", "-10% planet cost per level", O_UpgradeEffectType.PlanetCostReduction, 0.1, 14),
                CreateUpgrade("spark_of_life", "Spark of Life", "+30% life chance per level", O_UpgradeEffectType.LifeChance, 0.3, 22),
                CreateUpgrade("genetic_memory", "Genetic Memory", "+15% DNA from collapse per level", O_UpgradeEffectType.DnaMultiplier, 0.15, 25),
                CreateUpgrade("parallel_echo", "Parallel Echo", "+0.5 passive stardust/sec between runs", O_UpgradeEffectType.ParallelEcho, 0.5, 30)
            };

            catalog = ScriptableObject.CreateInstance<O_UpgradeCatalog>();
            catalog.upgrades = upgrades;
            AssetDatabase.CreateAsset(catalog, path);
            return catalog;
        }

        private static O_UpgradeDefinition CreateUpgrade(string id, string name, string desc, O_UpgradeEffectType type, double effect, double cost)
        {
            var path = $"{SoPath}/O_Upgrades/{id}.asset";
            var existing = AssetDatabase.LoadAssetAtPath<O_UpgradeDefinition>(path);
            if (existing != null)
                return existing;
        ...
        private static O_VariantCatalog CreateVariantCatalog()
        {
            var path = $"{SoPath}/O_VariantCatalog.asset";
            var catalog = AssetDatabase.LoadAssetAtPath<O_VariantCatalog>(path);
            if (catalog != null && catalog.variants != null && catalog.variants.Length > 0)
                return catalog;

            catalog = ScriptableObject.CreateInstance<O_VariantCatalog>();
        ...
        private static O_ParallelVariant CreateVariant(string id, string name, string bonus, string drawback,
            double stardust, double entropy, double life, double lifespan, double dna)
        {
            var path = $"{SoPath}/O_Variants/{id}.asset";
            var existing = AssetDatabase.LoadAssetAtPath<O_ParallelVariant>(path);
            if (existing != null)
                return existing;
        ...
        {
            var path = $"{PrefabPath}/O_Star.prefab";
            var existing = AssetDatabase.LoadAssetAtPath<O_StarView>(path);
            if (existing != null)
                return existing;
            ...
            return prefab.GetComponent<O_StarView>();
        }

        private static O_PlanetView CreatePlanetPrefab(Sprite sprite)
        {
            var path = $"{PrefabPath}/O_Planet.prefab";
            var existing = AssetDatabase.LoadAssetAtPath<O_PlanetView>(path);
            if (existing != null)
                return existing;
            ...
            var prefab = PrefabUtility.SaveAsPrefabAsset(go, path);
            Object.DestroyImmediate(go);
            return prefab.GetComponent<O_PlanetView>();
        }

        private static void CreateGameScene(O_GameBalance balance, O_UpgradeCatalog catalog, O_VariantCatalog variants, O_StarView starPrefab, O_PlanetView planetPrefab, Sprite sprite)
        {
            ...
            camGo.AddComponent<O_WorldCamera>();
            ...
            bootstrap.AddComponent<O_GameBootstrap>();
            bootstrap.AddComponent<O_GameClock>();
            bootstrap.AddComponent<O_SaveSystem>();
            ...
            SetSerialized(run, "gameClock", bootstrap.GetComponent<O_GameClock>());
            ...
            SetSerialized(bootstrap.GetComponent<O_GameBootstrap>(), "runController", run);
            ...
            var floatingText = canvasGo.AddComponent<O_FloatingTextSpawner>();
            SetSerialized(floatingText, "canvas", canvasGo.GetComponent<Canvas>());

            var hud = canvasGo.AddComponent<O_HUDController>();
        ...
        private static void BuildHudUi(Transform canvas, O_HUDController hud, O_UpgradePanelController upgrades, O_CollapsePanelController collapse, O_VariantPickerController variants)
        {
