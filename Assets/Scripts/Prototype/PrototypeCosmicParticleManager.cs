...
{
    public struct PrototypeParticleCollectResult
    {
    ...
    public class PrototypeCosmicParticleManager : MonoBehaviour
    {
        [SerializeField] private PrototypeGameController controller;
        [SerializeField] private PrototypeWorldParticle stardustParticlePrefab;
        [SerializeField] private PrototypeWorldParticle dnaParticlePrefab;
        ...
        private readonly List<PrototypeWorldParticle> _particles = new();
        ...
        public float GetCollectibleFlySpeed(int value) =>
            PrototypeParticleTiers.GetFlySpeed(value) * collectibleFlySpeedMultiplier;
        ...
        public void EmitStardustBurst(int totalAmount, Vector3 worldPosition, PrototypeStarStage stage,
            float particleEvolutionChance = 0f)
        {
            if (totalAmount <= 0)
                return;

            var chunks = PrototypeParticleTiers.SplitIntoChunks(totalAmount, particleEvolutionChance);
            for (var i = 0; i < chunks.Count; i++)
            {
                var chunk = chunks[i];
                var chunkStage = chunk >= 5
                    ? PrototypeParticleTiers.GetPreferredStageForTier(chunk)
                    : stage;
        ...
        public void SpawnStardustParticle(int value, Vector3 worldPosition, PrototypeStarStage stage, float spawnDelay = 0f)
        {
            if (stardustParticlePrefab == null)
            {
                Debug.LogWarning("Stardust particle prefab is not assigned on PrototypeCosmicParticleManager.", this);
                return;
            }
        ...
        public void SpawnDnaFragment(Vector3 worldPosition)
        {
            ...
            if (prefab == null)
            {
                Debug.LogWarning("DNA particle prefab is not assigned on PrototypeCosmicParticleManager.", this);
                return;
            }
        ...
        public void OnStardustCollected(int value, Vector3 position)
        {
            ...
            if (value >= 100 && Random.value < PrototypeCosmicBalance.MassiveParticleDnaChance)
        ...
        public void TickBlackHolePull(IReadOnlyList<PrototypeBlackHoleView> blackHoles)
        {
            _particles.RemoveAll(p => p == null);
            ...
            foreach (var particle in _particles)
            {
                ...
                foreach (var hole in blackHoles)
                {
                    ...
                    if (dist <= PrototypeCosmicBalance.BlackHoleParticlePullRadius && dist < closestDist)
                    {
                        closestDist = dist;
        ...
        public PrototypeParticleCollectResult CollectParticlesInRadius(Vector3 center, float radius)
        {
            _particles.RemoveAll(p => p == null);

            var result = new PrototypeParticleCollectResult();
