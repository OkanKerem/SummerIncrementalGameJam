...
{
    public class O_PlanetManager : MonoBehaviour
    {
        [SerializeField] private Transform worldRoot;
        [SerializeField] private O_PlanetView planetViewPrefab;

        private readonly List<O_Planet> _planets = new();
        private readonly Dictionary<int, O_PlanetView> _views = new();
        ...
        public IReadOnlyList<O_Planet> Planets => _planets;
        ...
        public void Initialize(O_GameBalance balance, O_RunModifiers modifiers, O_StardustWallet wallet, O_StarManager starManager, Transform root)
        {
            _balance = balance;
        ...
        public bool CanCreatePlanetOn(O_Star star)
        {
        ...
        public bool TryCreatePlanet(O_Star star)
        {
            ...
            var planet = new O_Planet(_nextId++, star.Id, orbitIndex, orbitAngle, orbitRadius);
        ...
        public void Tick(float deltaTime)
        {
            foreach (var planet in _planets)
            {
                ...
                if (star.Stage == O_StarStage.RedGiant)
                    planet.IsUnstable = true;

                var baseChance = star.Stage == O_StarStage.Orange
        ...
        public void HandleStarSupernova(O_Star star)
        {
            var toDestroy = new List<O_Planet>();
            foreach (var planet in _planets)
            {
        ...
        private void DestroyPlanet(O_Planet planet, bool countForStats)
        {
        ...
        private void HandleLifeDeveloped(O_Planet planet)
        {
            _lifePlanets++;
            if (_views.TryGetValue(planet.Id, out var view) && view != null)
                view.RefreshVisual();
        ...
        private void HandlePlanetDestroyed(O_Planet planet)
        {
            if (planet.HasLife)
        ...
        private void RefreshStarPlanetBonus(O_Star star)
        {
            double bonus = 1;
            foreach (var planet in _planets)
            {
        ...
        private O_Star GetStar(int starId)
        {
