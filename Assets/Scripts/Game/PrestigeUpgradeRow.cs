...
{
    public class PrestigeUpgradeRow : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        [SerializeField] private PrestigeUpgradeDefinition definition;
        ...
        public PrestigeUpgradeDefinition Definition => definition;
        ...
        public void Initialize(PrestigeState prestige, bool allowPurchase)
        {
            _controller = null;
            _prestige = prestige;
        ...
        private void InitializeCommon()
        {
            if (definition == null)
            {
                Debug.LogWarning($"PrestigeUpgradeRow on {name} has no definition assigned.", this);
                return;
            }
        ...
        public void Refresh()
        {
            ...
            if (effectText != null)
                effectText.text = PrestigeModifiers.DescribeEffect(definition, level);
        ...
        private string BuildTooltipText()
        {
            ...
            var maxed = definition.maxLevel > 0 && level >= definition.maxLevel;
            var unlocked = definition.ArePrerequisitesMet(_prestige, out var requirementText);
            var effect = PrestigeModifiers.DescribeEffect(definition, level);
